import { Availability, AvailabilityTemplate, AvailabilityException } from "../models/index.js";
import {
  getWeekStart,
  getWeekDates,
  parseDateUTC,
...
async function findTemplateRow(owner) {
  if (owner.role === "MENTOR") {
    return AvailabilityTemplate.findOne({ mentorId: owner.mentorId });
  }
  return AvailabilityTemplate.findOne({ userId: owner.userId });
}
...
export async function replaceTemplate(owner, patternSlots) {
  ...
  const base = { role: owner.role };

  if (owner.role === "MENTOR") {
    await AvailabilityTemplate.findOneAndUpdate(
      { mentorId: owner.mentorId },
      { $set: { slots }, $setOnInsert: { mentorId: owner.mentorId, ...base } },
      { upsert: true, new: true }
    );
  } else {
    await AvailabilityTemplate.findOneAndUpdate(
      { userId: owner.userId },
      { $set: { slots }, $setOnInsert: { userId: owner.userId, ...base } },
      { upsert: true, new: true }
    );
  }
...
async function getExceptionsForWeek(owner, weekStart) {
  ...
  return AvailabilityException.find(where).sort({ dayOfWeek: 1, hour: 1 });
}
...
async function ensureTemplateFromLegacyAvailabilities(owner) {
  ...
  const latest = await Availability.findOne(availWhere).sort({ date: -1 });
  if (!latest) return [];
  ...
  const rows = await Availability.find({ ...availWhere, date: { $gte: ws, $lt: weekEnd } }).sort({ date: 1, startTime: 1 });
...
async function upsertException(owner, weekStart, dayOfWeek, hour, enabled) {
  ...
  if (owner.role === "MENTOR") {
    return AvailabilityException.findOneAndUpdate(
      ...
      { $set: { enabled }, $setOnInsert: { mentorId: owner.mentorId, ...data } },
      { upsert: true, new: true }
    );
  }
  ...
  return AvailabilityException.findOneAndUpdate(
    ...
    { $set: { enabled }, $setOnInsert: { userId: owner.userId, ...data } },
    { upsert: true, new: true }
  );
}
...
async function deleteException(owner, weekStart, dayOfWeek, hour) {
  ...
  const where =
    owner.role === "MENTOR"
      ? { mentorId: owner.mentorId, weekStart: ws, dayOfWeek, hour }
      : { userId: owner.userId, weekStart: ws, dayOfWeek, hour };
  await AvailabilityException.deleteMany(where);
}
