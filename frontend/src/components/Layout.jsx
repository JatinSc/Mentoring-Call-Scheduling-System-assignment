...
const LOGIN_WELCOME_MODAL_KEY = "login_welcome_modal";
...
export default function Layout() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem(LOGIN_WELCOME_MODAL_KEY);
      if (!raw) return;
      const data = JSON.parse(raw);
      sessionStorage.removeItem(LOGIN_WELCOME_MODAL_KEY);
      setWelcomeModal({
        email: data.email || "—",
        role: data.role || "—",
      });
